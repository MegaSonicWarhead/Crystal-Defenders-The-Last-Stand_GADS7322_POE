...
namespace CrystalDefenders.Gameplay
{
    ...
    public class WaveManager : MonoBehaviour
    {
        ...
        [SerializeField] private int enemiesIncrementPerWave = 1;
        ...
        [SerializeField] private float spawnIntervalAccelerationPerWave = -0.03f;
        ...
        public float ResourceGainMultiplier => resourceGainMultiplier;
        private float resourceGainMultiplier = 1f;
        ...
        private IEnumerator SpawnWaveRoutine()
        {
            ...
            // Cap enemies per spawner to limit sudden surges
            toSpawnPerSpawner = Mathf.Min(toSpawnPerSpawner, 8);

            // --- Adaptive Non-linear Spawn Interval ---
            // Base exponential curve: starts at 4s (wave 1) → eases toward 1s (wave 20), min 0.75s.
            float decayRate = 0.15f;
            float baseInterval = 1f + (4f - 1f) * Mathf.Exp(-decayRate * (currentWave - 1));
            baseInterval = Mathf.Max(0.75f, baseInterval);

            // Adaptive scaling based on player performance
            float towerHealthFactor = Mathf.Clamp01(towerHealth / 1000f); // 0–1
            float struggleFactor = Mathf.Clamp01(1.2f - diff);             // Slower if struggling

            // Weighted influence of tower condition and defender pressure
            float adaptiveModifier = Mathf.Lerp(0.85f, 1.15f, (towerHealthFactor + defenderPressureAdj * 0.5f) / 1.5f);
            adaptiveModifier *= struggleFactor;

            // Apply adaptive modifier to interval
            float interval = baseInterval * Mathf.Clamp(adaptiveModifier, 0.75f, 1.25f);

            // Early wave slowdown for breathing room
            if (currentWave <= 2)
                interval *= earlyWaveSlowMultiplier;

            // Clamp final result
            interval = Mathf.Max(0.75f, Mathf.Min(interval, 6f));
            ...
            // --- Begin spawning enemies from all registered spawners ---
                ...
                spawner.BeginSpawning(
                    ...
                    enemy =>
                    {
                        ...
                        if (health != null)
                        {
                            ...
                            int baseHp = Mathf.Min(100, 10 + (currentWave - 1) * 6);
                            int hp = Mathf.RoundToInt(baseHp * diff);
                            health.SetMaxHealth(hp, true);
                        }
                    });
            }
        ...
        //    // --- Non-linear spawn interval progression ---
        //    // Starts at 4s (wave 1) → eases down to 1s (wave 20), min 0.75s.
        //    float decayRate = 0.15f;
        //    float interval = 1f + (4f - 1f) * Mathf.Exp(-decayRate * (currentWave - 1));
        //    interval = Mathf.Max(0.75f, interval);

        //    // Slight slowdown on first two waves to let players stabilize
        //    if (currentWave <= 2)
        //        interval *= earlyWaveSlowMultiplier;
        ...
        public void IncreaseResourceMultiplier(float factor)
        {
            resourceGainMultiplier *= factor;
        }
    }
}